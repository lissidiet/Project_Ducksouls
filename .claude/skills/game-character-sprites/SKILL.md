...
description: Create game-ready fixed-cell pixel-art character spritesheets from text concepts, reference images, or existing character art in Claude Code. Use when Claude Code needs to design, generate, clean up, validate, and package 2D game characters with native 32x32, 64x64, or 128x128 cells; 4-way or 8-way directions; idle, walk/run, jump, attack, archer, caster, or other animation strips; transparent GIF/WebP previews; codex-gateway-imagegen visual generation; and per-direction targeted regeneration.
---
...
Use this skill to make game-ready fixed-cell pixel-art character animations in Claude Code. Use the `codex-gateway-imagegen` skill for all visual generation. If the user names cell sizes, those sizes are the required output scope. If no size is named, default to `64x64`.
...
run/
  run-manifest.json
  source/<codex-gateway-imagegen-output>.png
  <cell>/
...
Do not ask `codex-gateway-imagegen` for a combined `32/64/128` contact sheet as the normal workflow. Combined multi-size contact sheets make the image generator treat the sizes as layout elements inside one picture, not as separate pixel-art budgets. For native assets, generate each requested size as its own `codex-gateway-imagegen` row strip at that size budget.
...
Use the `codex-gateway-imagegen` skill as the only visual generation backend. Do not use generic platform image generators, third-party art services, local drawing code, or text-only notes as substitutes for real art generation.

Required handoff from `codex-gateway-imagegen`:
...
6. For multi-size jobs, each requested size must have its own `codex-gateway-imagegen` output file. Do not use one generated contact sheet as the source for multiple native sizes.
7. Record every generated strip in `run/run-manifest.json` with `method: "codex-gateway-imagegen"`, cell size, action, direction, imagegen output path, source path, and prompt path.
...
For Claude Code, keep the workflow CLI and file based. Write artifacts to the workspace and return final paths for the atlas, metadata, contact sheet, validation JSON, and preview GIF/WebP files.
...
`codex-gateway-imagegen` output must become a local file before this skill can finish. Ask that skill to generate one PNG per action-direction-size strip, then copy the selected result into `run/source/` and leave the original in place when possible.

If `codex-gateway-imagegen` cannot expose the generated image as a local file, stop and say the image was generated but cannot be processed into sprite files. Do not claim the sprite pack is complete.

Normal native workflow:

```text
codex-gateway-imagegen -> run/source/32-walk-south.png -> run/32/generated/walk-south.png
codex-gateway-imagegen -> run/source/64-walk-south.png -> run/64/generated/walk-south.png
codex-gateway-imagegen -> run/source/128-walk-south.png -> run/128/generated/walk-south.png
```

Each source image must be generated specifically for that cell budget. For example, the `32` prompt should ask for one row of six `32x32` frames only. The `64` prompt should ask for one row of six `64x64` frames only. The `128` prompt should ask for one row of six `128x128` frames only.
...
run/source/<codex-gateway-imagegen-output>.png
...
A run is not a valid art test unless the visual source strips came from `codex-gateway-imagegen` using the user's reference. Text-only identity notes are not enough.
...
- `generation.method` is `procedural`, `python_draw`, `script`, `text_only`, or anything other than `codex-gateway-imagegen`
- `generation.imagegen_output_path` is missing or not a real local file
- one `codex-gateway-imagegen` output is reused as the source for multiple requested native sizes without explicit fallback approval
...
Use the `codex-gateway-imagegen` skill for visual generation. Do not hand-draw missing animation frames with local code. Scripts may crop, assemble, quantize, snap, validate, and preview already-generated art.
...
- run manifest does not prove `codex-gateway-imagegen` reference provenance
