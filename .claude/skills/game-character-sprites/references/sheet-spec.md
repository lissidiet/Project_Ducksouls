...
- Real art tests must use the `codex-gateway-imagegen` skill grounded in the user's reference image.
...
- `codex-gateway-imagegen` output must be available as a local file and copied into `run/source/`.
- Multi-size contact sheets from `codex-gateway-imagegen` are diagnostic fallback only. Native delivery requires separate generated outputs for each requested cell size.
