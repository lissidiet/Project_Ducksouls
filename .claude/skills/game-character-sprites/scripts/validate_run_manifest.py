...
def method_is_imagegen(value: Any) -> bool:
    ...
    return method == "codex-gateway-imagegen"
...
def main() -> None:
    ...
    if not isinstance(generation, dict):
        ...
        if not method_is_imagegen(generation.get("method")) and not args.allow_procedural:
            errors.append(f"generation.method must be codex-gateway-imagegen, got {generation.get('method')!r}")
        ...
        if method_is_imagegen(generation.get("method")):
            ...
            if imagegen_output_path is None:
                errors.append("generation.imagegen_output_path is required for codex-gateway-imagegen runs")
    ...
    for index, strip in enumerate(strips):
        ...
        if not method_is_imagegen(strip.get("method")) and not args.allow_procedural:
            errors.append(f"strip {key} method must be codex-gateway-imagegen, got {strip.get('method')!r}")
    ...
    if not (isinstance(generation, dict) and generation.get("imported_contact_sheet") is True):
        for source, used_sizes in source_usage.items():
            if len(used_sizes) > 1:
                errors.append(
                    "one codex-gateway-imagegen output is reused for multiple native sizes "
